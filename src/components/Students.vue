  ...
  <h1>Searching</h1>
      ...
      <table>
        <tr v-for="(stud, index) in students" :key="index">
          ...
          <td>{{ stud.group }}</td>
          <td>{{ stud.grade }}</td>
          <td><a @click="deleteStudent(stud._id)">Delete</a></td>
        </tr>
      </table>

      <div class="addStudent">
        <input placeholder="add name" v-model="newStudent.name" />
        <input v-model="newStudent.workDone" type="checkbox" />
        <select v-model="newStudent.group" id="myComboBox" name="group">
          <option value="placeholder" disabled selected>Group:</option>
...
export default {
    data() {
       return {
           students: [],
           searchName:'',
           newStudent: {id:"" ,name: "", workDone: false, group: ""}
       }
    },
    mounted () {
    ...
    methods: {
     ...
     axios.delete(`http://34.82.81.113:3000/students/${id}`).then(data => {
      ...
      axios.post("http://34.82.81.113:3000/students", this.newStudent).then(response => {console.log(response.data);
